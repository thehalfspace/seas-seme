...
# ╔═╡ a5dd971d-0f49-4880-a623-ccc5242a959c
md"""
## 📈 Plot 2: Vfmax with Event Detection

Highlighting earthquake events (Vfmax > threshold).
"""

# ╔═╡ b5dd971d-0f49-4880-a623-ccc5242a959d
let
	figsize = (900, 400)
	fig = Figure(resolution=figsize)
	...
	ax = Axis(fig[1, 1],
			 ...
			 ylabel="Max. Slip Rate (m/s)",
			 yscale=log10,
			 title="Maximum Slip Rate with Events")

	# Plot slip rate
	lines!(ax, times_years, Vfmax, color=:blue, linewidth=2.5, label="Vfmax")

	# Identify and highlight events
	event_threshold = 1e-3  # m/s
	event_mask = Vfmax .>= event_threshold
	event_times = times_years[event_mask]
	event_vfmax = Vfmax[event_mask]

	if length(event_times) > 0
		scatter!(ax, event_times, event_vfmax, color=:red, markersize=10, label="Events")
	end

	# Add threshold line
	hlines!(ax, [event_threshold], color=:red, linestyle=:dash, linewidth=1.5,
		   label="Threshold")

	axislegend(ax, position=:rt)

	save(joinpath(fig_dir, "vfmax_events.png"), fig)
	fig
end

# ╔═╡ c5dd971d-0f49-4880-a623-ccc5242a959e
md"""
## 📈 Plot 3: Earthquake Cycle Heatmap

Slip rate vs depth and time showing the spatiotemporal evolution of earthquakes.
"""

# ╔═╡ d5dd971d-0f49-4880-a623-ccc5242a959f
let
	# Read slip rate matrix for all fault nodes
	times_hm, slip_rate_matrix, depths_hm = h5open(h5_file, "r") do file
		t = read(file, "timeseries/time")
		d = read(file, "mesh/fault_depths_km")
		n_depths = length(d)
		n_times = length(t)
		...
		# Initialize matrix
		sr_matrix = zeros(n_depths, n_times)
		...
		for depth_str in depth_groups
			...
			sr_matrix[fault_idx, :] .= slip_rate
		end

		return t, sr_matrix, d
	end

	# Filter by depth range
	depth_range = (0.0, 20.0)
	mask = (depths_hm .>= depth_range[1]) .& (depths_hm .<= depth_range[2])
	slip_rate_filtered = slip_rate_matrix[mask, :]
	depths_filtered = depths_hm[mask]
	...
	hm = heatmap!(ax, 1:length(times_hm), depths_filtered, slip_rate_filtered,
				 colormap=:inferno,
				 colorrange=(1e-9, 1e0),
				 colorscale=log10,
				 interpolate=true)
	...
	save(joinpath(fig_dir, "eq_cycle_heatmap.png"), fig)
	fig
end

# ╔═╡ e5dd971d-0f49-4880-a623-ccc5242a960a
md"""
## 📈 Plot 4: Combined Vfmax + Heatmap

Two-panel plot showing Vfmax time series aligned with the heatmap.
"""

# ╔═╡ f5dd971d-0f49-4880-a623-ccc5242a960b
let
	# Read slip rate matrix
	times_hm, slip_rate_matrix, depths_hm = h5open(h5_file, "r") do file
		t = read(file, "timeseries/time")
		d = read(file, "mesh/fault_depths_km")
		n_depths = length(d)
	...
	times_hm_years = times_hm ./ yr2sec
	...
	lines!(ax1, times_years, Vfmax, color=:blue, linewidth=2.5)
	...
	hm = heatmap!(ax2, times_hm_years, depths_filtered, slip_rate_filtered,
				 colormap=:inferno,
				 colorrange=(1e-9, 1e0),
				 colorscale=log10,
				 interpolate=true)
	...
	save(joinpath(fig_dir, "vfmax_with_heatmap.png"), fig)
	fig
end

# ╔═╡ 05dd971d-0f49-4880-a623-ccc5242a960c
md"""
## 📈 Plot 5: Slip Rate at Specific Depth

Time series of slip rate at a chosen depth location.
"""

# ╔═╡ 15dd971d-0f49-4880-a623-ccc5242a960d
let
	# Choose a depth to plot
	target_depth = 10.0  # km

	# Read slip rate at this depth
	depth_str = @sprintf("depth_%.1fkm", target_depth)

	times_depth, slip_rate_depth = h5open(h5_file, "r") do file
		t = read(file, "timeseries/time")
		sr = read(file, "timeseries/$depth_str/slip_rate")
		return t, sr
	end

	times_depth_years = times_depth ./ yr2sec
	...
	ax = Axis(fig[1, 1],
			 ...
			 yscale=log10,
			 title="Slip Rate at $(target_depth) km depth")

	lines!(ax, times_depth_years, slip_rate_depth, color=:blue, linewidth=2.5)

	save(joinpath(fig_dir, "sliprate_depth_$(target_depth)km.png"), fig)
	fig
end

# ╔═╡ 25dd971d-0f49-4880-a623-ccc5242a960e
md"""
## 📈 Plot 6: Cumulative Slip Evolution
...
let
	...
	times_slip, cumulative_slip = h5open(h5_file, "r") do file
		t = read(file, "timeseries/time")
		slip = read(file, "timeseries/$depth_str/slip")
		return t, slip
	end

	times_slip_years = times_slip ./ yr2sec
	...
	ax = Axis(fig[1, 1],
			 ...
			 title="Cumulative Slip at $(target_depth) km depth")

	lines!(ax, times_slip_years, cumulative_slip, color=:green, linewidth=2.5)

	save(joinpath(fig_dir, "cumulative_slip_$(target_depth)km.png"), fig)
	fig
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "1da93b9efa21ab699fb3d7cccff0007aaf329327"
