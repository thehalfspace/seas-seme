...
function main_loop()
	...
	# Calculate jacobian matrix and metric terms 
	for el in 1:mesh.n_elements

		if mesh.element_is_curved[el]
			# Curved side quad mapping (pre/mapping_geometry_curved_2d.jl)
			calc_curved_node_coordinates!(node_coordinates, el, basis.nodes, view(mesh.surface_curves, :, el))
			calc_curved_metric_terms!(jac_matrix, el, basis.nodes, view(mesh.surface_curves, :, el))
			#calc_normal_directions!(node_coordinates, el, basis.nodes, view(mesh.surface_curves, :, el))

		else
			# Straight side quad mapping (pre/mappings_geometry_straight_2d.jl)
	...
	fault_id, fault_x, fault_y, fault_matrix = 
		get_boundary_nodes_unstructured(mesh, node_coordinates, jac_matrix, basis.weights, 1, dof_id, :fault)
	creep_id, creep_x, creep_y, creep_matrix = 
		get_boundary_nodes_unstructured(mesh, node_coordinates, jac_matrix, basis.weights, 1, dof_id, :creep)
	absorbing_id, absorbing_x, absorbing_y, absorb_matrix = 
		get_boundary_nodes_unstructured(mesh, node_coordinates, jac_matrix, basis.weights, ρ*vs, dof_id, :absorbing)
	...
	fault_matrix[1] = fault_matrix[1] + creep_matrix[end]
