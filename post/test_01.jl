...
md"""
## Scripts to plot results from simulation output using CairoMakie.jl
"""

# ╔═╡ 8fbc466b-17ef-4b1e-b0f9-d7aaeeb18c7a
md"""
#### Plot Mesh and Initial Conditions
"""
...
begin
	...
	out_path = joinpath(dirname(pwd()), "data/", simulation_name)
	mesh_path = joinpath(out_path, "mesh/")
	fig_path = joinpath(dirname(pwd()), "plots/", simulation_name)
...
filename: pwd()/data/simulation_name/initial\__conditions.out 
"""

# ╔═╡ 05bb5cbd-e6db-45be-95c8-9ec5d4da6dbb
# Read the file contents
begin
	ics = readdlm(joinpath(out_path, "initial_conditions.out"), skipstart=1);
	fault_depth = ics[:,1]
	σo = ics[:,2]
	τo = ics[:,3]
	a = ics[:,4]
...
# ╔═╡ 0c03f79e-5e6f-4066-b719-e1c4b6cea3d6
...
# Plot initial stresses and friction with depth
# let block creates local scope
let
	figsize = (4.2, 3.35) # in inches
	f = Figure(resolution=figsize.*dpi)
	ax = Axis(f[1,1],
			title="Initial Conditions on fault",
			xlabel="Stresses (MPa)",
			ylabel="Depth (km)")
	
	lines!(ax, σo, fault_depth, label="Normal Stress")
	lines!(ax, τo, fault_depth, label="Shear Stress")
	ylims!(ax, 20,0)
	axislegend(ax, position = :lb, labelsize=36)
	save(joinpath(fig_path, "initial_conditions.png"), f)
	f
end
...
# ╔═╡ 29cb4348-a871-413a-a3ed-c48265fa3404
md"""
#### Max. Sliprate and Cumulative Slip
"""

# ╔═╡ 5050cf67-1fa9-4306-b911-dd80825d1cd1
# Read sliprate data
begin
...
# ╔═╡ 4461c7e0-b2f9-44bf-aef8-3f49a53f73ea
# Plot max sliprate
let
	figsize = (7.2, 3.35) # in inches
	f = Figure(resolution=figsize.*dpi)
	ax = Axis(f[1,1],
...
# ╔═╡ d5e99ac8-926d-4487-bbc4-6646da4fafb1


# ╔═╡ 4fe162f8-e203-4fa7-832e-e2c79f0e0243
# Read cumulative slip data
begin
	slip_data = readdlm(joinpath(out_path, "time_series_1d.out"), skipstart=1)
...
git-tree-sha1 = "1d75daa0a844360f6a65d735100e0329c2bcf6e4"
...
git-tree-sha1 = "86dceba662af551b813879d89e68d8e9342f725d"
